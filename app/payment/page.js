...
const PaymentModal = ({
  ...
  //     const customerResponse = await axios.get(`http://localhost:5000/api/customer/get-customer-by-mobile/${formData.mobileNumber}`);
  ...
  //         await axios.patch(`http://localhost:5000/api/customer/update-credit-balance/${existingCustomer._id}`, {
  ...
  //   // Add a new customer
  //   const newCustomerResponse = await axios.post('http://localhost:5000/api/customer/customer/list/add-customer', {
  //     customerName: formData.customerName,
  //     mobileNumber: formData.mobileNumber,
  ...
  const addNewCustomer = async () => {
    ...
    if (!formData.mobileNumber || formData.mobileNumber.length < 10) {
      console.error("Mobile number should be at least 10 digits. Payment aborted.");
      return;
    }
  ...
  const handleChange = (e) => {
    ...
    if (name === "customerName" && /[^a-zA-Z]/.test(value)) {
      ...
      console.error("Invalid input: Customer name must contain only alphabets.");
      return;
    }
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={handleChange}
                      autoComplete="off"
                      className="w-md p-2 border border-gray-300 rounded-md focus:outline-none focus:border-yellow-500"
